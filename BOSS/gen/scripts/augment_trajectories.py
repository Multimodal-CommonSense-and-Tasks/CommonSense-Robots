...
def augment_traj(env, json_file):
    ...
    if not os.path.exists(augmented_json_file):
        ...
        if args.only_reward_relabel or args.save_augmented_traj:
...
parser.add_argument(
    "--save_augmented_traj",
    action="store_true",
    help="save the actual augmented jsons. Don't use if just checking rewards or something, but make sure to enable it when actually creating a new dataset",
)
parser.add_argument(
