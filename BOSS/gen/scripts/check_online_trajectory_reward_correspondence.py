...
non_moveable_object = json.loads(open("../scene_sampling/non_moveable_object.json").read())
...
def augment_traj(env, json_file):
    ...
    if not os.path.exists(augmented_json_file):
        ...
        def generate_skill_info(traj_data, index):
            ...
            skill_info_for_online_env["annotations"] = traj_data["turk_annotations"]["anns"][0]["high_descs"][index]
        ...
        for ll_idx, ll_action in enumerate(traj_data["plan"]["low_actions"]):
            ...
            if prev_hl_action != hl_action:
                ...
                if (curr_hl_rewards[-1] < 1 or sum(curr_hl_rewards) > 1):
                    ...
                    video_name  = video_name.translate(str.maketrans("", "", string.punctuation)) 
                    ...
                    if prev_skill_info["args_object"][-1] in moveable_object:
                        if skill_valid[0] :
                            sub_goal_save_path = os.path.join("../direct_graph", "fail","moveable","valid" ,video_name.replace(" ", "_"))+".mp4"
                        else:
                            sub_goal_save_path = os.path.join("../direct_graph", "fail","moveable","invalid" ,video_name.replace(" ", "_"))+".mp4"
                    else:
                        if skill_valid[0] : 
                            sub_goal_save_path = os.path.join("../direct_graph", "fail","non_moveable","valid" ,video_name.replace(" ", "_"))+".mp4"
                        else:
                            sub_goal_save_path = os.path.join("../direct_graph", "fail","non_moveable","invalid" ,video_name.replace(" ", "_"))+".mp4"
                    writer = cv2.VideoWriter(
                        sub_goal_save_path, cv2.VideoWriter_fourcc(*"mp4v"), 5, (300, 300), True
                    )
                    ...
                    for frame_num in range(len(log_to_video)):
                        ...
                        non_graph_dis = non_moveable_agent_object_graph_distance[frame_num]
                    ...
                    if prev_skill_info["args_object"][-1] in moveable_object:
                        ...
                        sub_goal_save_path = os.path.join("../direct_graph", "success","moveable" ,video_name.replace(" ", "_"))+".mp4"
                    else:

                        sub_goal_save_path = os.path.join("../direct_graph", "success","non_moveable" ,video_name.replace(" ", "_"))+".mp4"
            ...
            non_moveable_agent_object_graph_distance.append(env._current_action.non_moveable_agent_object_graph_distance)
            moveable_agent_object_graph_distance.append(env._current_action.moveable_agent_object_graph_distance)
