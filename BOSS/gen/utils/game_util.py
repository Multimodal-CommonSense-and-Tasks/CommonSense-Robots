...
def object_type_equals(object_type, object_type_2):
    if object_type is None and object_type_2 is not None:
        return False
    elif object_type is not None and object_type_2 is None:
        return False
    # special case for knife
    elif "knife" in object_type.lower() and "knife" in object_type_2.lower():
        return True
    return object_type.lower() == object_type_2.lower()


def get_pose(event):
    pose = event.pose
    return (
        int(np.round(pose[0] / (1000 * constants.AGENT_STEP_SIZE))),
...
def get_camera_matrix(pose, camera_height):
    ...
    x_rotation = np.matrix([[1, 0, 0], [0, cos_x, -sin_x], [0, sin_x, cos_x]])
    sin_y = np.sin(pose[2] * np.pi / 180)
    cos_y = np.cos(pose[2] * np.pi / 180)
    y_rotation = np.matrix([[cos_y, 0, sin_y], [0, 1, 0], [-sin_y, 0, cos_y]])
    ...
    extrinsic_matrix = np.concatenate(
        (
            np.concatenate((rotation_matrix, np.matrix([0, 0, 0])), axis=0),
...
def get_rotation_matrix(pose):
    ...
    x_rotation = np.matrix(
        [[1, 0, 0], [0, cos_x, -sin_x], [0, sin_x, cos_x]], dtype=np.float32
    )
    sin_y = np.sin((-pose[2] % 4) * 90 * np.pi / 180)
    cos_y = np.cos((-pose[2] % 4) * 90 * np.pi / 180)
    y_rotation = np.matrix(
        [[cos_y, 0, sin_y], [0, 1, 0], [-sin_y, 0, cos_y]], dtype=np.float32
    )
    rotation_matrix = np.matmul(x_rotation, y_rotation)
...
def depth_to_world_coordinates(depth, pose, camera_height):
    x_points = np.arange(
        -constants.SCREEN_WIDTH / 2, constants.SCREEN_WIDTH / 2, dtype=depth.dtype
    )
    x_vals = depth * x_points / constants.FOCAL_LENGTH

    y_points = np.arange(
        constants.SCREEN_HEIGHT / 2, -constants.SCREEN_HEIGHT / 2, -1, dtype=depth.dtype
    )
    y_vals = (depth.T * y_points / constants.FOCAL_LENGTH).T

    z_vals = depth
    xyz = np.stack((x_vals, y_vals, z_vals), axis=2) / (
        1000 * constants.AGENT_STEP_SIZE
    )
...
def world_to_camera_coordinates(coordinates, pose, camera_height):
    ...
    y_points = constants.SCREEN_HEIGHT - (
        xyd[:, 1] * constants.FOCAL_LENGTH / depth + constants.SCREEN_HEIGHT / 2
    )
...
def get_templated_action_str(plan, idx=0):
    ...
    (
        ...
        next_object_name,
        next_recep_name,
    ) = get_relevant_objs(action, plan, idx)

    a_type = action["action"]
    templated_str = ""

    if "GotoLocation" in a_type:
        templated_str = "go to the %s" % (
            next_recep_name if next_recep_name != "" else prev_object_name
        )
...
def get_discrete_hl_action(plan, idx=0):
    ...
    if "GotoLocation" in a_type:
        ...
        discrete_action["args"] = [
            next_recep_name if next_recep_name != "" else next_object_name
        ]
    elif "OpenObject" in a_type:
...
def get_relevant_objs(action, plan, idx=0):
    object_name = (
        object_id_to_name(action["objectId"]).lower() if "objectId" in action else ""
    )
    recep_name = (
        object_id_to_name(action["receptacleObjectId"]).lower()
    ...
    if prev_idx >= 0:
        ...
        prev_object_name = (
            object_id_to_name(prev_action["objectId"]).lower()
            if "objectId" in prev_action
            else ""
        )
        prev_recep_name = (
            object_id_to_name(prev_action["receptacleObjectId"]).lower()
            if "receptacleObjectId" in prev_action
            else ""
        )
    ...
    if next_idx < len(plan):
        ...
        next_object_name = (
            object_id_to_name(next_action["objectId"]).lower()
            if "objectId" in next_action
            else ""
        )
        next_recep_name = (
            object_id_to_name(next_action["receptacleObjectId"]).lower()
            if "receptacleObjectId" in next_action
            else ""
        )
...
def get_objects_of_type(object_type, metadata):
    return [
        obj
        for obj in metadata["objects"]
        if object_type_equals(obj["objectType"], object_type.lower())
    ]
...
def get_objects_with_name_and_prop(name, prop, metadata):
    return [obj for obj in metadata["objects"] if name in obj["objectId"] and obj[prop]]
...
def get_object_bounds(obj, scene_bounds):
    ...
    obj_bounds[[0, 2]] = np.clip(
        obj_bounds[[0, 2]], scene_bounds[0], scene_bounds[0] + scene_bounds[2]
    )
    obj_bounds[[1, 3]] = np.clip(
        obj_bounds[[1, 3]], scene_bounds[1], scene_bounds[1] + scene_bounds[3]
    )
...
def get_task_str(object_ind, receptacle_ind=None, toggle_ind=None, mrecep_ind=None):
    ...
    recep = (
        constants.OBJECTS[receptacle_ind].lower() if receptacle_ind is not None else ""
    )
...
def store_image_name(name):
    constants.data_dict["images"].append(
        {
            "high_idx": get_last_hl_action_index(),
