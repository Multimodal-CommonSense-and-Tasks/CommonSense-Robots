...
from ET.et_iql_feeder import ETIQLFeeder, collate_func
...
def evaluate_val_dataloader(agent_model, dataloader, use_pretrained_lang, device):
    agent_model.eval()
    ret_dict = {}
    with torch.no_grad():
        ...
        for _, data_dict in enumerate(dataloader):
            ...
            loss_info = agent_model.train_offline_from_batch(
                frames,
                word_lang if use_pretrained_lang else lang,
                action,
                obj_id,
                lengths_frames,
                lengths_lang,
...
def train_epoch(agent_model, dataloader, use_pretrained_lang, epoch, device, save_dir, experiment_name):
    agent_model.train()
    with tqdm(dataloader, unit="batch") as tepoch:
        for batch_i, data_dict in enumerate(tepoch):
            ...
            stuff_to_log = dict(batch_i=batch_i, **loss_info,)
        ...
        # if none, just save the last epoch
...
def main(config):
    ...
    valid_seen_dataset = ETIQLFeeder(
    ...
    valid_unseen_dataset = ETIQLFeeder(
    ...
    if config.use_pretrained_lang:
        agent_model = ETIQLFixedEmbeddingModel(config)
    else:
        agent_model = ETIQLModel(config)
    ...
    if len(config.gpus) > 1:
        ...
        agent_model = nn.DataParallel(agent_model, device_ids=config.gpus,)
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        for name, valid_dataloader in zip(
            ...
            ret_dict = evaluate_val_dataloader(agent_model, valid_dataloader, config.use_pretrained_lang, device,)
            eval_metrics = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--discount", type=float, default=0.97, help="discount factor for the reward",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        const=True,
        nargs="?",
        help="train from primitive skills or composite skills",
    )
    parser.add_argument(
        "--max_skill_length",
        type=int,
    ...
    parser.add_argument(
        "--clip_score", type=float, default=100, help="max to clip the advantage to",
    )
    parser.add_argument(
        "--quantile", type=float, default=0.8, help="quantile for computing the reward",
    )
    parser.add_argument(
        "--advantage_temp",
        type=float,
    ...
    parser.add_argument(
        ...
        const=True,
        nargs="?",
        help="use more advanced critic mlps (layernorm and 3 layers)",
    )
    parser.add_argument(
        "--use_pretrained_lang",
        type=str2bool,
        ...
        const=True,
        nargs="?",
        help="use pretrained language encoder for the tokens (used for unsupervised RL training)",
    )
    ...
    config.encoder_lang = {
        "shared": True,
        "layers": base_config.transformer_layers,
        "pos_enc": True,
        "instr_enc": False,
    }
