...
class MVP(object):
    def __init__(self, args, eval=True, share_memory=False, use_conv_feat=True):
        ...
        if args.gpu is not None:
            if type(args.gpu) is bool:
                self.model = self.model.to(torch.device("cuda"))
            else:
                self.model = self.model.to(torch.device(args.gpu))
    ...
    def get_default_transform():
        return transforms.Compose(
            [
                transforms.Resize((256, 256)), # 256 instead of 224 for vit-l
                transforms.ToTensor(),
...
class Resnet(object):
    def __init__(self, args, eval=True, share_memory=False, use_conv_feat=True):
        ...
        if self.model_type == "maskrcnn":
            ...
            self.resnet_model = Resnet18(
                args, eval, share_memory, use_conv_feat=use_conv_feat
            )
        elif self.model_type == "resnet50":
            self.resnet_model = Resnet50(
                args, eval, share_memory, use_conv_feat=use_conv_feat
            )
        elif self.model_type == "mvp":
            self.resnet_model = MVP(
                args, eval, share_memory, use_conv_feat=use_conv_feat
            )
        else:
        ...
        self.transform = self.get_default_transform() if not hasattr(self.resnet_model, "get_default_transform") else self.resnet_model.get_default_transform()
    ...
    def featurize(self, images, batch=32):
        ...
        if self.gpu is not None:
            if type(self.gpu) is bool:
                images_normalized = images_normalized.to(torch.device("cuda"))
            else:
                images_normalized = images_normalized.to(torch.device(self.gpu))
