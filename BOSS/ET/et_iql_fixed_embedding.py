...
from ET.encodings import DatasetLearnedEncoding
...
class ETOfflineRLBaseModel(BaseModel):
    def embed_lang(self, sentences):
        """
        take a list of annotation tokens and extract embeddings with EncoderLang
        """
        lengths_lang = torch.tensor([1], dtype=torch.long).repeat(len(sentences))
        if isinstance(sentences, torch.Tensor):
            return sentences.unsqueeze(1), lengths_lang
        with torch.no_grad():
            ...
            #lengths_lang = torch.tensor([len(s) for s in emb_lang], dtype=torch.long)
            #emb_lang = pad_sequence(
            #    emb_lang, batch_first=True, padding_value=0
            #)  # .to(self.args.gpus[0])
...
class ETIQLPolicy(ETOfflineRLBaseModel):
    def __init__(self, args, vocab_word, encoder_lang, pad=0, seg=1):
        """
    ...
    def forward(self, **inputs):
        ...
        # embed language
        output = {}
        emb_lang, lengths_lang = self.embed_lang(inputs["lang"])
...
class ETIQLCritics(ETOfflineRLBaseModel):
    ...
    def get_value(self, frames, lang):
        ...
        # embed language
        emb_lang, lengths_lang = self.embed_lang(lang)
    ...
    def forward(self, forward_v, **inputs):
        """
        forward the model for multiple time-steps (used for training)
        """
        # embed language
    ...
    def perform_model_update(self, losses, grad_scaler):
        ...
        # grad_scaler.scale(losses["q1_loss"]).backward(retain_graph=True)
        # grad_scaler.scale(losses["q2_loss"]).backward(retain_graph=True)
        # grad_scaler.scale(losses["vf_loss"]).backward()
...
class ETIQLFixedEmbeddingModel(nn.Module):
    def __init__(self, args, pad=0, seg=1):
        """
        transformer IQL agent
        """
        super(ETIQLFixedEmbeddingModel, self).__init__()
        sentence_transformer = SentenceTransformer(
            "all-mpnet-base-v2", device=torch.device(args.gpus[0])
        )
        ...
        self.policy = ETIQLPolicy(args, self.vocab_word, sentence_transformer, pad, seg)

        self.critics = ETIQLCritics(
            args, self.vocab_word, sentence_transformer, pad, seg
        )
    ...
    def train_offline_from_batch(
        ...
        obj_id,
        lengths_frames,
        lengths_lang, # unused, for compatibility
        interact_mask,
        rewards,
        terminals,
        eval=False,
    ...
    def step(self, input_dict, ret_value):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            assert frames_traj_pad.size(1) == action_traj_pad.size(1)
            assert frames_traj_pad.size(1) == obj_traj_pad.size(1)
