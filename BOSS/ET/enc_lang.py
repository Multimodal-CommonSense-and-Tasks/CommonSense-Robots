...
from ET.encodings import PosLangEncoding, InstrLangEncoding
...
class EncoderLang(nn.Module):
    def __init__(
        ...
        if args.encoder_lang["shared"]:
            ...
            self.enc_transformers = nn.ModuleDict(
                {data: enc_transformer for data in embs_ann.keys()}
            )
        else:
            self.enc_transformers = nn.ModuleDict(
                {
        ...
        # encodings
        self.enc_pos = (
            PosLangEncoding(args.demb) if args.encoder_lang["pos_enc"] else None
        )
        self.enc_instr = (
            InstrLangEncoding(args.demb) if args.encoder_lang["instr_enc"] else None
        )
    ...
    def forward(self, lang_pad, embedder, vocab, pad):
        ...
        mask_token = EncoderLang.mask_token(
            lang_pad, vocab, {}
        )  # remove subgoal and goal_token as we don't have higher level instructions
    ...
    def encoder(encoders, emb_lang, mask_pad, vocab, mask_attn=None):
        ...
        if mask_attn is None:
            ...
            mask_attn = torch.zeros(
                (mask_pad.shape[1], mask_pad.shape[1]), device=mask_pad.device
            ).float()
        # encode the inputs
        output = encoders[vocab.name](
            emb_lang.transpose(0, 1), mask_attn, mask_pad
        ).transpose(0, 1)
        return output
