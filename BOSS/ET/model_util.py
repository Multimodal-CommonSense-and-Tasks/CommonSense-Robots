...
import ET.metric_util as metric_util
...
def extract_action_preds(
    model_out, pad, vocab_action, clean_special_tokens=True, lang_only=False, offset=0
):
    """
    output processing for a VLN agent
    """
    zipped_data = zip(
        (model_out["action"].max(2)[1] + offset).tolist(), model_out["object"]
    )
...
def compute_obj_class_precision(metrics, gt_dict, classes_out):
    ...
    interact_idxs = (
        gt_dict["action_valid_interact"].reshape(-1).nonzero(as_tuple=False).reshape(-1)
    )
    obj_classes_prob = classes_out.reshape(
        classes_out.shape[0] * classes_out.shape[1], *classes_out.shape[2:]
    )[interact_idxs]
    ...
    obj_classes_gt = gt_dict["object"]
    obj_classes_gt_inter = obj_classes_gt.reshape(
        obj_classes_gt.shape[0] * obj_classes_gt.shape[1]
    )[interact_idxs]
    precision = torch.sum(obj_classes_pred == obj_classes_gt_inter) / len(
        obj_classes_gt_inter
    )
    metrics["action/object"].append(precision.item())


def obj_classes_loss(pred_obj_cls, gt_obj_cls, interact_idxs, weights=None):
    """
    Compute a cross-entropy loss for the object class predictions.
    """
    pred_obj_cls_inter = pred_obj_cls[interact_idxs]
    gt_obj_cls_inter = gt_obj_cls[interact_idxs]
    # the interaction objects should be non zeros
    assert not (gt_obj_cls_inter == 0).any()
    # compute the loss for interaction objects
    if weights is None:
        obj_cls_loss = F.cross_entropy(
            pred_obj_cls_inter, gt_obj_cls_inter, reduction="mean"
        )
    else:
        obj_cls_loss = F.cross_entropy(
            pred_obj_cls_inter, gt_obj_cls_inter, reduction="none"
        )
        obj_cls_loss *= weights[interact_idxs]
        obj_cls_loss = obj_cls_loss.mean()
    return obj_cls_loss


def tokens_to_lang(tokens, vocab, skip_tokens=None, join=True, offset=0):
    """
    ...
    def _tokens_to_lang(seq):
        if isinstance(seq, torch.Tensor):
            seq = seq.tolist()
        lang = [vocab.index2word(t + offset) for t in seq if t not in skip_tokens]
