...
class CICETIQLModel(ETIQLFixedEmbeddingModel):
    ...
    def __init__(self, args, pad=0, seg=1):
        ...
        self.cic_opt = torch.optim.Adam(list(self.cic.parameters()) + list(self.encoder.parameters()), lr=self.args.lr["final"])
        self.cic.train()
    ...
    def train_offline_from_batch(self, frames, skill, action, obj_id, lengths_frames, interact_mask, rewards, terminals, eval=False):
        ...
        skill_flattened = skill.unsqueeze(1).repeat(1, seq_length, 1).reshape(-1, self.skill_dim)
        ...
        metrics.update(super().train_offline_from_batch(frames, skill, action, obj_id, lengths_frames, None, interact_mask, intr_reward, terminals, eval))
        return metrics
