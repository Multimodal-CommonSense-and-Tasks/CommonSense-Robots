...
import torch
from eval_utils import process_skill_strings, add_prefix_to_skill_string
import random
...
class SaycanPlanner(LargeLanguageModel):
    ...
    #prompt += (
    #    starter
    #    + "put the pencil on the desk in the mug and place it on the desk in front of the computer?\n"
    #)
    #prompt += "Robot: "
    #prompt += "1. Pick up the pencil on the desk, nearest to the clock.\n"
    #prompt += "2. Put the pencil in the mug on the desk.\n"
    #prompt += "3. Pick up the mug from the desk.\n"
    #prompt += "4. Place the mug on the desk, in front of the computer.\n"

    #prompt += starter + "place a chilled apple in a microwave?\n"
    #prompt += "Robot: "
    #prompt += "1. Pick up the apple on the counter.\n"
    #prompt += "2. Place the apple in the fridge and close the door. wait a moment and then take the apple out.\n"
    #prompt += "3. Place the apple in the microwave.\n"

    #prompt += starter + "put the towel from the towel ring into the sink?\n"
    #prompt += "Robot: "
    #prompt += "1. Take the towel from the towel ring on the wall.\n"
    #prompt += "2. Put the towel down in the sink.\n"

    #prompt += (
    #    starter
    #    + "put a washed cup from the fridge onto the left edge of the cabinet?\n"
    #)
    #prompt += "Robot: "
    #prompt += "1. Open the fridge, pick up the green cup next to the tomato and close the fridge.\n"
    #prompt += "2. Put the cup in the sink, turn on the water for a few seconds, turn it off and pick up the cup again.\n"
    #prompt += "3. Put the cup on the left edge of the cabinet.\n"

    #prompt += (
    #    starter + "put two sets of keys, from the couch and table, on the table?\n"
    #)
    #prompt += "Robot: "
    #prompt += "1. Pick up the keys that are closest to the edge, in the center of the couch.\n"
    #prompt += (
    #    "2. Place the keys on the corner of the table, closest to the fireplace.\n"
    #)
    #prompt += "3. Pick up the keys close to the edge of the shelf.\n"
    #prompt += (
    #    "4. Place the keys on the table, between the other set of keys and the watch.\n"
    #)

    #prompt += (
    #    starter + "warm a slice of bread in the microwave and place it in the bin?\n"
    #)
    #prompt += "Robot: "
    #prompt += "1. Pick up the knife off of the table.\n"
    #prompt += "2. Slice the bread on the table.\n"
    #prompt += "3. Open the cupboard and place the knife inside.\n"
    #prompt += "4. Pick up the slice of bread.\n"
    #prompt += "5. Open the microwave and heat the slice of bread.\n"
    #prompt += "6. Place the toast inside the bin.\n"

    #prompt += starter + "look at the clock, from on top of the desk, under the light?\n"
    #prompt += "Robot: "
    #prompt += "1. Pick up the clock towards the back on top of the desk.\n"
    #prompt += "2. Turn on the lamp.\n"
    prompt += starter
    prompt_mid_1 = "Robot: "
    ...
    def __init__(self, config):
        ...
        config.generate_next_skill_codex = False
        config.llm_next_skill_temp = 0.8
        config.llm_summary_temp = None
        super().__init__(config)
        self.lock = Semaphore(config.llm_batch_size)
        self.threshold = 0.95
        self.skill_match_with_dataset = config.skill_match_with_dataset
    ...
    def preprocess_llm_inputs_for_logprob(
        ...
        for (prompt_part_one, prompt_part_two, high_level_task) in zip(
            ...
            only_part_two = (
                ...
                second_with_mid_annotations[2:] # get rid of the number (" 1." for example)
            )
    ...
    def _get_logprobs(
        ...
        for i in range(0, len(sample_second_annotations), self.llm_batch_size):
            (
                ...
                all_first_annotations[i: i + self.llm_batch_size], sample_second_annotations[i: i+self.llm_batch_size], high_level_tasks_repeated[i: i+self.llm_batch_size],
            )
            batch_input_ids = batch_tokenized_prompt_no_end.input_ids
            batch_attention_mask = batch_tokenized_prompt_no_end.attention_mask
            batch_second_part_attention_mask = batch_tokenized_second_part.attention_mask
    ...
    def _get_logprobs_by_skill_match(
        ...
        while not found_next_skill:
            next_skills, next_logprobs = self._generate_hf_text(
                all_tokenized_prompts, num_generations=5, 
            )
            #next_skills, _ = self._generate_hf_text(
            #    all_tokenized_prompts, num_generations=5, ret_logprobs=False
            #)
            matched_skill_indicies, index_in_next_skill, new_threshold = self.search_for_next_skill(
        ...
        #matched_skills = [sample_second_annotations[i] for i in matched_skill_indicies]
        matched_logprobs = [next_logprobs[index_in_next_skill[i]] for i in range(len(matched_skill_indicies))]
        #matched_logprobs = self._get_logprobs(
        #    first_annotation, matched_skills, high_level_tasks
        #)
